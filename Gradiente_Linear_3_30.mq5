//+------------------------------------------------------------------+
//|                                   Gradiente Linear v3.30 VOLUMES |
//|                                  Copyright 2025, Trading Expert  |
//|         ✅ v3.30: DETECÇÃO DE TENDÊNCIA (BOLLINGER)                |
//+------------------------------------------------------------------+
...
//v3.28: ESTOQUE MÁXIMO - Insere ordem Stop de proteção quando posições atingem limite configurado
//v3.28: STOP no ponto médio entre última executada e próxima pendente
...
// ✅ v3.29: Ação específica para stop loss (independente da meta de lucro)
enum ENUM_ACAO_STOP {
   ACAO_STOP_DESATIVAR = 0,   // Fechar tudo e desativar
   ACAO_STOP_RECRIAR = 1,     // Fechar e recriar grade
   ACAO_STOP_COOLDOWN = 2     // Cooldown com backoff exponencial
};

// ✅ v3.29: Ação ao detectar tendência via Bollinger
enum ENUM_ACAO_BOLLINGER {
   ACAO_BB_FECHAR_TUDO = 0,       // Fechar ordens, posições e desativar
   ACAO_BB_FECHAR_ORDENS = 1,     // Fechar somente ordens e desativar
   ACAO_BB_MANTER_ORDENS = 2      // Desativar mantendo ordens
};
...
input group "■■■ Meta de Lucro (Apenas HEDGE) "
...
input group "■■■ Estoque Máximo (Stop de Proteção) "
input int      EstoqueMaximo = 0;              // Estoque máximo de posições (0 = desabilitado)
...
input group "■■■ Ação Após Stop Loss (PrejuízoMáximo) "
input double   PrejuizoMaximo = 0;         // Limite de prejuízo (R$) - 0 = desabilitado
input ENUM_ACAO_STOP AcaoAposStopLoss = ACAO_STOP_DESATIVAR;  // Ação após stop loss
input int      PausaInicialMinutos = 30;             // Cooldown: Pausa inicial (minutos)
input int      MaxTentativasConsecutivas = 3;        // Cooldown: Máx tentativas antes de desativar
input bool     ReusarGradeAposPausa = false;         // Cooldown: Reusar grade anterior ao retomar

input group "■■■ Detecção de Tendência (Bandas de Bollinger) "
input bool     UtilizarBollinger = false;                  // Ativar detecção por Bollinger
input ENUM_TIMEFRAMES BBTimeframe = PERIOD_M5;             // Timeframe das Bandas
input int      BBPeriodo = 20;                             // Período da média
input double   BBDesvio = 2.0;                             // Desvio padrão
input double   BBInclinacaoMax = 0.05;                     // Inclinação máx da média (%)
input int      BBInclinacaoPeriodo = 5;                    // Candles para cálculo da inclinação
input double   BBLarguraMax = 0.5;                         // Largura máx das bandas (%)
input int      BBRompimentoCandles = 3;                    // Candles consecutivos fora da banda
input ENUM_ACAO_BOLLINGER AcaoBollinger = ACAO_BB_FECHAR_TUDO;  // Ação ao detectar tendência
...
// ✅ v3.29: Controle de Cooldown após Stop Loss
bool     emCooldown                = false;
datetime inicioCooldown            = 0;
int      tentativaAtual            = 0;
int      tempoCooldownSegundos     = 0;

// ✅ v3.29: Controle de Detecção de Tendência (Bollinger)
int      handleBollinger           = INVALID_HANDLE;
bool     emPausaBollinger          = false;
bool     bbInclinacaoBloqueando    = false;
bool     bbLarguraBloqueando       = false;
bool     bbRompimentoBloqueando    = false;
...
//+------------------------------------------------------------------+
//| ✅ v3.28: Contar posições abertas do EA (HEDGE e NETTING)        |
//+------------------------------------------------------------------+
int ContarPosicoesAbertas()
{
   int contador = 0;

   if(isHedge)
   {
      // HEDGE: Conta posições filtradas por MagicNumber
   ...
   {
      // NETTING: Posição consolidada - conta volume em lotes
      if(PositionSelect(_Symbol))
      {
         ...
         if(volume > 0 && volumeMin > 0)
            contador = (int)MathRound(volume / volumeMin);
      }
   }
...
//+------------------------------------------------------------------+
//| ✅ v3.28: Obter ticket da posição mais antiga (HEDGE)            |
//+------------------------------------------------------------------+
ulong ObterPosicaoMaisAntiga()
{
   ulong ticketMaisAntigo = 0;
   datetime tempoMaisAntigo = D'2099.01.01';
   ...
   for(int i = 0; i < PositionsTotal(); i++)
   {
      ...
      if(ticket > 0)
      {
         if(PositionGetString(POSITION_SYMBOL) == _Symbol &&
            PositionGetInteger(POSITION_MAGIC) == MagicNumber)
         {
            datetime tempoAbertura = (datetime)PositionGetInteger(POSITION_TIME);

            if(tempoAbertura < tempoMaisAntigo)
            {
               tempoMaisAntigo = tempoAbertura;
...
void InserirOrdemStop(double precoUltimaExecutada, ENUM_ORDER_TYPE tipoUltimaExecutada)
{
   ...
   if(tipoUltimaExecutada == ORDER_TYPE_BUY_LIMIT)
   {
      ...
      {
         if(originalOrders[i].orderType == ORDER_TYPE_BUY_LIMIT &&
            originalOrders[i].originalPrice < precoUltimaExecutada)
         {
            if(originalOrders[i].originalPrice > maiorPreco)
            {
               maiorPreco = originalOrders[i].originalPrice;
               indiceProxima = i;
            }
         }
      }
   ...
   else if(tipoUltimaExecutada == ORDER_TYPE_SELL_LIMIT)
   {
      ...
      for(int i = 0; i < ArraySize(originalOrders); i++)
      {
         if(originalOrders[i].orderType == ORDER_TYPE_SELL_LIMIT &&
            originalOrders[i].originalPrice > precoUltimaExecutada)
         {
            if(originalOrders[i].originalPrice < menorPreco)
            {
               menorPreco = originalOrders[i].originalPrice;
               indiceProxima = i;
            }
         }
      }
...
//+------------------------------------------------------------------+
//| ✅ v3.29: Atualizar condições das Bandas de Bollinger            |
//| Calcula: inclinação, largura e rompimento                        |
//+------------------------------------------------------------------+
void AtualizarCondicoesBollinger()
{
   if(handleBollinger == INVALID_HANDLE) return;

   int necessario = MathMax(BBInclinacaoPeriodo + 1, BBRompimentoCandles + 1);
   ...
   if(CopyBuffer(handleBollinger, 0, 0, necessario, middle) < necessario) return;
   if(CopyBuffer(handleBollinger, 1, 0, necessario, upper) < necessario) return;
   if(CopyBuffer(handleBollinger, 2, 0, necessario, lower) < necessario) return;
   if(CopyClose(_Symbol, BBTimeframe, 0, necessario, close) < necessario) return;

   // 1. INCLINAÇÃO: variação % da média em N candles
   double inclinacao = 0;
   if(middle[BBInclinacaoPeriodo] != 0)
      inclinacao = (middle[0] - middle[BBInclinacaoPeriodo]) / middle[BBInclinacaoPeriodo] * 100.0;
   bbInclinacaoBloqueando = (MathAbs(inclinacao) > BBInclinacaoMax);

   // 2. LARGURA: (upper - lower) / middle * 100
   double largura = 0;
   if(middle[0] != 0)
      largura = (upper[0] - lower[0]) / middle[0] * 100.0;
   bbLarguraBloqueando = (largura > BBLarguraMax);

   // 3. ROMPIMENTO: candles consecutivos fechando fora da banda (a partir do último fechado)
   int consecutivosFora = 0;
   for(int i = 1; i <= BBRompimentoCandles; i++)
   {
      if(close[i] > upper[i] || close[i] < lower[i])
         consecutivosFora++;
      else
         break;
   }
   bbRompimentoBloqueando = (consecutivosFora >= BBRompimentoCandles);

   if(ModoDebug && (bbInclinacaoBloqueando || bbLarguraBloqueando || bbRompimentoBloqueando))
   {
      Print("📊 BB Debug - Incl: ", DoubleToString(inclinacao, 4), "% | Larg: ",
            DoubleToString(largura, 4), "% | Fora: ", consecutivosFora, " candles");
   }
}

//+------------------------------------------------------------------+
//| ✅ v3.29: Verificar Bollinger (quando EA está ativo)             |
//| Se qualquer condição disparar → pausa                            |
//+------------------------------------------------------------------+
void VerificarBollinger()
{
   if(!UtilizarBollinger || emPausaBollinger) return;

   AtualizarCondicoesBollinger();

   if(bbInclinacaoBloqueando || bbLarguraBloqueando || bbRompimentoBloqueando)
   {
      Print("========================================");
      Print("📊 BOLLINGER: TENDÊNCIA DETECTADA!");
      if(bbInclinacaoBloqueando) Print("  ↗️ Inclinação da média acima do limite");
      if(bbLarguraBloqueando) Print("  ↔️ Largura das bandas acima do limite");
      if(bbRompimentoBloqueando) Print("  ⚡ Rompimento: candles consecutivos fora da banda");
      Print("Ação: ", EnumToString(AcaoBollinger));
      Print("========================================");

      EntrarPausaBollinger();
   }
}

//+------------------------------------------------------------------+
//| ✅ v3.29: Entrar em pausa por Bollinger                          |
//+------------------------------------------------------------------+
void EntrarPausaBollinger()
{
   emPausaBollinger = true;

   // Desativar EA
   robotAtivo = false;

   switch(AcaoBollinger)
   {
      case ACAO_BB_FECHAR_TUDO:
         // Fechar ordens e posições
         CancelarTodasOrdensDoEA(); Sleep(300);
         FecharPosicoes(); Sleep(500);
         CancelarTodasOrdensDoEA(); Sleep(300);

         // Terceira passada de segurança
         {
         ...
         Print("📊 BB: Ordens e posições fechadas. EA desativado.");
         break;

      case ACAO_BB_FECHAR_ORDENS:
         // Fechar apenas ordens (mantém posições abertas)
         CancelarTodasOrdensDoEA(); Sleep(300);
         CancelarTodasOrdensDoEA(); Sleep(300);
         Print("📊 BB: Ordens fechadas (posições mantidas). EA desativado.");
         break;

      case ACAO_BB_MANTER_ORDENS:
         // Não fecha nada, apenas desativa
         Print("📊 BB: EA desativado mantendo ordens e posições.");
         break;
   }
...
//+------------------------------------------------------------------+
//| ✅ v3.29: Verificar retomada após pausa por Bollinger            |
//| Retoma APENAS quando as 3 condições estiverem livres             |
//+------------------------------------------------------------------+
void VerificarRetomadaBollinger()
{
   if(!emPausaBollinger) return;

   AtualizarCondicoesBollinger();

   // Só retoma se TODAS as 3 condições estiverem livres
   if(bbInclinacaoBloqueando || bbLarguraBloqueando || bbRompimentoBloqueando)
   {
      AtualizarStatusVisual();
      return;
   ...
   // TODAS AS CONDIÇÕES LIVRES → RETOMAR
   Print("========================================");
   Print("📊 BOLLINGER: Condições normalizadas! Retomando operações...");
   Print("========================================");

   emPausaBollinger = false;
   bbInclinacaoBloqueando = false;
   bbLarguraBloqueando = false;
   bbRompimentoBloqueando = false;

   robotAtivo = true;
   AtualizarStatusVisual();

   // Usar configuração de persistência de grade para retomar
   string nomeArquivo = StringFormat("%s_%d_Grade.csv", _Symbol, MagicNumber);
   if(IniciarGradeAnterior && FileIsExist(nomeArquivo, FILE_COMMON))
   {
      Print("📂 Retomando com grade anterior do CSV");
      CarregarGradeAnterior();
   }
   else
   {
      Print("🔄 Criando nova grade ao retomar");
      CriarGradeInicialCompleta();
   }
}

//+------------------------------------------------------------------+
//| ✅ v3.29: Entrar em modo Cooldown após stop loss                 |
//+------------------------------------------------------------------+
void EntrarCooldown()
{
   tentativaAtual++;

   if(tentativaAtual > MaxTentativasConsecutivas)
   {
      // Esgotou tentativas → desativa EA para o dia
      Print("========================================");
      Print("🚫 COOLDOWN: ", tentativaAtual - 1, " stops consecutivos. EA DESATIVADO para o dia.");
      Print("========================================");
   ...
   int multiplicador = 1;
   for(int i = 1; i < tentativaAtual; i++)
      multiplicador *= 2;

   tempoCooldownSegundos = PausaInicialMinutos * 60 * multiplicador;
   emCooldown = true;
   inicioCooldown = TimeCurrent();

   Print("========================================");
   Print("⏸️ COOLDOWN ATIVADO! Tentativa ", tentativaAtual, "/", MaxTentativasConsecutivas);
   Print("Pausa de ", tempoCooldownSegundos / 60, " minutos");
   Print("Retorno previsto: ", TimeToString(inicioCooldown + tempoCooldownSegundos, TIME_DATE|TIME_SECONDS));
   Print("========================================");

   // Fechar tudo e pausar
   CancelarTodasOrdensDoEA(); Sleep(300);
   FecharPosicoes(); Sleep(500);
   CancelarTodasOrdensDoEA(); Sleep(300);
...
//+------------------------------------------------------------------+
//| ✅ v3.29: Verificar se cooldown expirou                          |
//+------------------------------------------------------------------+
void VerificarCooldown()
{
   ...
   int tempoDecorrido = (int)(agora - inicioCooldown);

   if(tempoDecorrido < tempoCooldownSegundos)
      return;

   // COOLDOWN EXPIROU → RETOMAR OPERAÇÕES
   Print("========================================");
   Print("▶️ COOLDOWN EXPIROU! Retomando operações...");
   Print("Tentativa ", tentativaAtual, "/", MaxTentativasConsecutivas);
   Print("========================================");

   emCooldown = false;

   // Resetar controle de lucro/prejuízo para novo ciclo
   ultimoResetLucroPrejuizo = TimeCurrent();
   breakEvenAtivado = false;
   prejuizoDinamico = 0;
   nivelBaseTrailing = 0;

   // Limpar array e recriar grade
   ArrayResize(originalOrders, 0);

   if(ReusarGradeAposPausa)
   {
      string nomeArquivo = StringFormat("%s_%d_Grade.csv", _Symbol, MagicNumber);
      if(FileIsExist(nomeArquivo, FILE_COMMON))
      {
         Print("📂 Reusando grade anterior do CSV");
         CarregarGradeAnterior();
      }
      else
      {
         Print("📂 CSV não encontrado, criando nova grade");
         CriarGradeInicialCompleta();
      }
   }
   else
...
void VerificarMetaLucroPrejuizo()
{
   ...
   if(LucroAlvo > 0 && lucroPrejuizoAtual >= LucroAlvo)
   {
      ...
      // ✅ v3.29: Lucro atingido reseta contagem de tentativas de cooldown
      tentativaAtual = 0;
   ...
   else
   {
      ...
      if(PrejuizoMaximo < 0 && lucroPrejuizoAtual <= PrejuizoMaximo)
      {
         ...
         // ✅ v3.29: Ação específica para stop loss (independente do lucro)
         if(AcaoAposStopLoss == ACAO_STOP_COOLDOWN)
         {
            Print("🔄 Ação: Cooldown com backoff exponencial");
            EntrarCooldown();
         }
         else
         {
            ExecutarAcaoStopLoss();
         }
         return;
      }
   }
...
//+------------------------------------------------------------------+
//| ✅ v3.29: Executar ação específica para Stop Loss                |
//| Independente de AcaoAoAtingirMeta (que controla apenas o lucro)  |
//+------------------------------------------------------------------+
void ExecutarAcaoStopLoss()
{
   Print("Executando ação stop loss: ", EnumToString(AcaoAposStopLoss));

   // Se desativar, marca ANTES de fechar (evita OnTradeTransaction criar ordens)
   if(AcaoAposStopLoss == ACAO_STOP_DESATIVAR)
   {
      robotAtivo = false;
      metaAtingidaHoje = true;
      AtualizarStatusVisual();
      ...
      if(ModoDebug)
         Print("🛑 Robô desativado ANTES de fechar posições/ordens (stop loss)");
   }

   // Fechar tudo (mesma lógica de ExecutarAcaoMeta)
   Print("Cancelando todas as ordens pendentes (1ª passada)...");
   CancelarTodasOrdensDoEA();
   Sleep(300);
   ...
   if(AcaoAposStopLoss == ACAO_STOP_DESATIVAR)
   {
      ...
      Print("✅ EA DESATIVADO após stop loss");
      Print("🚫 Reativação automática BLOQUEADA para hoje");
      if(InicioAutomatico)
         Print("Será reativado apenas no PRÓXIMO DIA às ", HorarioInicio);
      else
   ...
   else if(AcaoAposStopLoss == ACAO_STOP_RECRIAR)
   {
      ultimoResetLucroPrejuizo = TimeCurrent();
      Print("📊 Lucro/Prejuízo resetado. Novo período inicia em: ", TimeToString(ultimoResetLucroPrejuizo));
      ...
      Print("Criando nova grade após stop loss...");
      Sleep(1000);
      CriarGradeInicialCompleta();

      Print("✅ NOVA GRADE CRIADA após stop loss");
      Print("EA continua ativo e operando");
   }
...
{
   // ✅ v3.29: Não sincronizar durante cooldown ou pausa BB
   if(emCooldown || emPausaBollinger)
   {
      Print("⏸️ Sincronização ignorada - EA em pausa");
      return;
   }
...
{
   // ✅ v3.29: Mostrar estado de cooldown
   if(emCooldown)
   {
      int tempoRestante = tempoCooldownSegundos - (int)(TimeCurrent() - inicioCooldown);
      if(tempoRestante < 0) tempoRestante = 0;
      string texto = StringFormat("Cooldown %d/%d (%dmin)",
                     tentativaAtual, MaxTentativasConsecutivas, tempoRestante / 60);
      ObjectSetString(0, "Panel_Status", OBJPROP_TEXT, texto);
      ObjectSetInteger(0, "Panel_Status", OBJPROP_COLOR, clrOrangeRed);
   }
   // ✅ v3.29: Mostrar estado de pausa Bollinger
   else if(emPausaBollinger)
   {
      string motivos = "";
      if(bbInclinacaoBloqueando) motivos += "Incl ";
      if(bbLarguraBloqueando) motivos += "Larg ";
      if(bbRompimentoBloqueando) motivos += "Romp ";
      string texto = "BB Pausa: " + motivos;
      ObjectSetString(0, "Panel_Status", OBJPROP_TEXT, texto);
      ObjectSetInteger(0, "Panel_Status", OBJPROP_COLOR, clrMagenta);
   }
   else if(robotAtivo)
   {
...
void VerificarHorarioInicio()
{
   ...
   if(diaUltimaVerificacao != tempo.day)
   {
      ...
      // ✅ v3.29: Resetar cooldown para novo dia
      emCooldown = false;
      tentativaAtual = 0;
      tempoCooldownSegundos = 0;
...
void OnClickBtnIniciar()
{
   ...
   // ✅ v3.29: Reset manual do cooldown
   if(emCooldown)
   {
      Print("📢 Usuário forçou reativação durante cooldown");
      emCooldown = false;
      tentativaAtual = 0;
      tempoCooldownSegundos = 0;
   }

   // ✅ v3.29: Reset manual da pausa Bollinger
   if(emPausaBollinger)
   {
      Print("📢 Usuário forçou reativação durante pausa Bollinger");
      emPausaBollinger = false;
      bbInclinacaoBloqueando = false;
      bbLarguraBloqueando = false;
      bbRompimentoBloqueando = false;
...
int OnInit()
{
   ...
   // ✅ v3.29: Inicializar indicador Bollinger
   if(UtilizarBollinger)
   {
      handleBollinger = iBands(_Symbol, BBTimeframe, BBPeriodo, 0, BBDesvio, PRICE_CLOSE);
      if(handleBollinger == INVALID_HANDLE)
      {
         Print("❌ Erro ao criar indicador Bollinger: ", GetLastError());
         return INIT_FAILED;
      }
      Print("============================================");
      Print("📊 Detecção de Tendência (Bollinger) ATIVADA");
      Print("Timeframe: ", EnumToString(BBTimeframe));
      Print("Período: ", BBPeriodo, " | Desvio: ", DoubleToString(BBDesvio, 1));
      Print("Inclinação máx: ", DoubleToString(BBInclinacaoMax, 4), "% (", BBInclinacaoPeriodo, " candles)");
      Print("Largura máx: ", DoubleToString(BBLarguraMax, 4), "%");
      Print("Rompimento: ", BBRompimentoCandles, " candles consecutivos");
      Print("Ação: ", EnumToString(AcaoBollinger));
      Print("============================================");
   }
   ...
   {
      Print("============================================");
      Print("✅ Gradient Grid v3.29 VOLUMES inicializado!");
      Print("✅ v3.29: Cooldown + Bollinger");
...
void OnDeinit(const int reason)
{
   ...
   // ✅ v3.29: Liberar handle do Bollinger
   if(handleBollinger != INVALID_HANDLE)
...
void OnTick()
{
   // ✅ v3.29: Bollinger - verificar retomada mesmo com EA inativo
   if(emPausaBollinger && !robotAtivo)
   {
      VerificarRetomadaBollinger();
      if(!robotAtivo) { ChartRedraw(); return; }
   }

   if(InicioAutomatico && !robotAtivo && !emPausaBollinger)
   {
   ...
   // ✅ v3.29: Se em cooldown, verificar se expirou e pular verificação de meta
   if(emCooldown)
   {
      VerificarCooldown();
      ChartRedraw();
   ...
   // ✅ v3.29: Bollinger - verificar tendência quando EA está ativo
   if(UtilizarBollinger)
   {
      VerificarBollinger();
      if(!robotAtivo) { ChartRedraw(); return; }
                        ...
                        const MqlTradeResult& result)
{
   ...
   // ✅ v3.29: Bloquear processamento de deals durante cooldown ou pausa BB
   if(emCooldown || emPausaBollinger)
      return;
   ...
   if(ModoDebug)
   {
      ...
      if(EstoqueMaximo > 0)
         Print("Posições abertas: ", ContarPosicoesAbertas(), " / Máximo: ", EstoqueMaximo);
      Print("========================================");
   }
}
